/**
 * Import ScamSniffer blacklist domains into DOSafe threat_intel.
 *
 * domains.json is ~8.7MB with 300k+ domains — too large for Supabase Edge Function
 * 150s timeout. Runs via GitHub Actions (sync-large-sources.yml) every 6h.
 *
 * Usage:
 *   deno run --allow-net --allow-env scripts/import-scamsniffer.ts
 *
 * Env vars:
...
const SOURCE = "scamsniffer_domains";
...
try {
  console.log("Fetching ScamSniffer blacklist domains...");
  const res = await fetch(DOMAINS_URL, { signal: AbortSignal.timeout(120000) });
  if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);

  const domains: string[] = await res.json();
  const uniqueDomains = [...new Set(domains.map((d: string) => d.toLowerCase()))];
  console.log(`ScamSniffer: ${domains.length} blacklist domains → ${uniqueDomains.length} unique`);
